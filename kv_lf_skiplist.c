...
static unsigned shard_ix(uint64_t k, int ns) {
...
static KVMap *create_impl(int nshards, int unused) {
    (void)unused;
    if (nshards < 16) nshards = 256;
    KVMap *m = (KVMap *)calloc(1, sizeof(KVMap));
    if (!m) return NULL;
    m->nshards = nshards;
    m->heads = (LNode **)calloc((size_t)nshards, sizeof(LNode *));
    if (!m->heads) {
        free(m);
        return NULL;
    }
...
static int put_impl(KVMap *m, uint64_t key, const void *val) {
    unsigned s = shard_ix(key, m->nshards);
    LNode *head = m->heads[s];
    for (;;) {
        LNode *pred, *curr;
...
static int get_impl(KVMap *m, uint64_t key, void *out) {
    unsigned s = shard_ix(key, m->nshards);
    LNode *pred, *curr;
    harris_find(m->heads[s], key, &pred, &curr);
    if (curr && curr->key == key) {
...
const KVMapVTable kv_vtable_lf_skip = {
    .create = create_impl,
    .destroy = destroy_impl,
    .put = put_impl,
    .get = get_impl,
    .del = del_impl,
    .name = "lf_sharded_harris_sorted",
};
