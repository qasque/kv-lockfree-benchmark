...
static KVMap *create_impl(int nbuckets, int unused) {
    ...
    m->locks = (SRWLOCK *)malloc((size_t)nbuckets * sizeof(SRWLOCK));
    if (!m->heads || !m->locks) {
        free(m->heads);
        free(m->locks);
        free(m);
...
static int put_impl(KVMap *m, uint64_t key, const void *val) {
    ...
    AcquireSRWLockExclusive(&m->locks[b]);
    Node *p = m->heads[b];
    while (p) {
        if (p->key == key) {
            memcpy(p->val, val, KV_VALUE_BYTES);
            ReleaseSRWLockExclusive(&m->locks[b]);
            return 1;
        }
        p = p->next;
    }
...
static int get_impl(KVMap *m, uint64_t key, void *out) {
    unsigned b = bucket_ix(key, m->nbuckets);
    AcquireSRWLockShared(&m->locks[b]);
    for (Node *p = m->heads[b]; p; p = p->next) {
        if (p->key == key) {
            memcpy(out, p->val, KV_VALUE_BYTES);
            ReleaseSRWLockShared(&m->locks[b]);
            return 1;
        }
    }
    ReleaseSRWLockShared(&m->locks[b]);
...
const KVMapVTable kv_vtable_rwlock = {
    .create = create_impl,
